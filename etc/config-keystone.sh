...
mysql -uroot -p${DATABASE_PASSWORD} -e "CREATE DATABASE keystone;" &&  mysql -uroot -p${DATABASE_PASSWORD} -e "GRANT ALL PRIVILEGES ON keystone.* TO 'keystone'@'localhost' IDENTIFIED BY '${ALL_PASSWORD}';" && mysql -uroot -p${DATABASE_PASSWORD} -e "GRANT ALL PRIVILEGES ON keystone.* TO 'keystone'@'%' IDENTIFIED BY '${ALL_PASSWORD}';"   
