...
mysql -uroot -p${DATABASE_PASSWORD} -e "CREATE DATABASE glance;" &&  mysql -uroot -p${DATABASE_PASSWORD} -e "GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'localhost' IDENTIFIED BY '${ALL_PASSWORD}';" && mysql -uroot -p${DATABASE_PASSWORD} -e "GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'%' IDENTIFIED BY '${ALL_PASSWORD}';" 
